/**
 * Computes the display code for a subject on the client side.
 ...
 */
export const computeSubjectCode = (subject, branch, admissionYear = null) => {
  if (!subject) return '';
  const branchObj = typeof branch === 'object' ? branch : null;
  const rawCode = branchObj?.code || branchObj?.name || (typeof branch === 'string' ? branch : 'SUB');
  const branchCode = String(rawCode || 'SUB')
    .replace(/[^A-Za-z0-9]/g, '')
    .toUpperCase();
...
export default computeSubjectCode;
